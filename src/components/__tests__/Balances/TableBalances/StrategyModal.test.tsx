...
import StrategyModal from '../../../Balances/TableBalances/StrategyModal';
...
describe('StrategyModal', () => {
  test('test that StrategyModal is not rendered if openModal is false', () => {
    const props = {
      openModal: false,
      handleCloseModal: jest.fn(),
      data: strategyInfo,
    };
    const { queryByTestId } = render(<StrategyModal {...props} />);
    expect(queryByTestId('modal-identifier')).toBeNull();

  });
  test('test that StrategyModal is rendered if openModal is true and render props', () => {
    const props = {
      openModal: true,
      handleCloseModal: jest.fn(),
      data: strategyInfo,
    };
    const { getByTestId, getByText } = render(<StrategyModal {...props} />);
    expect(getByTestId('modal-identifier')).toBeInTheDocument();
    expect(getByText(props.data.vault.name)).toHaveTextContent(props.data.vault.name);
    expect(getByText(props.data.balance.toFixed(2))).toHaveTextContent(props.data.balance.toFixed(2));
    expect(getByText(props.data.vault.asset)).toHaveTextContent(props.data.vault.asset);
    expect(getByText(`${props.data.myBoost.toFixed(2)}%`)).toHaveTextContent(`${props.data.myBoost.toFixed(2)}%`);
    expect(getByText(`${props.data.yearlyRoi}%`)).toHaveTextContent(`${props.data.yearlyRoi}%`);
    expect(getByTestId('table-earned-tokens').children.length).toBe(props.data.settBalance.earnedTokens.length);
  });
});