...
import TableBalances, { TableBalancesProps } from '../../../Balances/TableBalances';
...
describe('TableBalances', () => {
  const props: TableBalancesProps = {
    target: 'strategy',
    payload: {
      cardHead: {
        total: 51522.221302,
  ...
  test('tests that the table is rendered according to the target and checks that the lengths of the elements and the children of the table match', () => {
    const { queryAllByTestId } = render(<TableBalances {...props} />);
    expect(queryAllByTestId('asset-table-row').length).toBe(0);
    expect(queryAllByTestId('strategy-table-row').length).toBe(props.payload.balancesData.length);
  });
  test('tests if clicking on a row opens the StrategyModal component and close button work', () => {
    const { queryAllByTestId, queryByTestId, getByTestId } = render(<TableBalances {...props} />);
    expect(queryByTestId('modal-identifier')).toBeNull();
    fireEvent.click(queryAllByTestId('strategy-table-row')[0]);
